...
export class Controller extends gridCon.GridController<any> {
    ...
    constructor() {
        ...
        //console.log(this.$stateParams['one']);
        this.isBusy = true;
        this.$q.all({
    ...
    detailInterface(d, item, needParam) {
        ...
        if (d['interface']['isLocal']) {
            ...
            if(this.$stateParams['two']){
                this.queryData["code"] = this.$stateParams['two'];
            }else if(this.$stateParams['one']){
                this.queryData["code"] = this.$stateParams['one'];
            } 
