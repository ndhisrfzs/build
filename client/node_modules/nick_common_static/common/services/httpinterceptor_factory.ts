            ...
            'request': function (httpConfig) {
                ...
                if (httpConfig.needToken === true) {
                    if (httpConfig.url.indexOf('?') > 0) {
                        httpConfig.url += '&token=' + $cookieStore.get(config.prefix + 'access_token');
                    } else {
                        httpConfig.url += '?token=' + $cookieStore.get(config.prefix + 'access_token');
                    }
                }
            ...
            'response': function (response) {
                if (response.status == 200 && response.data instanceof Object) {
                    if(response.data.ret_code){
                        response.data.result_code = parseInt(response.data.ret_code);
                    }
                    if (angular.isNumber(response.data.result_code) && response.data.result_code !== 1) {
                        ...
                        //默认显示错误信息
                        $rootScope.$emit("showError", "error", response.data.message);
