...
function mdDataTable() {
    ...
    function Controller($attrs, $element, $q, $scope, $rootScope) {
        var self = this;
        ...
        self.singleSelected = $attrs.hasOwnProperty("mdSingleRowSelect") && $attrs.mdSingleRowSelect === 'true';
        ...
        self.defer = function () {
            ...
            $rootScope.$emit('loading', self.deferred);
            ...
            if ($attrs.mdRowSelect && ngRepeat) {
                self.listener = $scope.$parent.$watch(ngRepeat.items, function (newValue, oldeValue) {
                    if (newValue !== oldeValue) {
                        !self.singleSelected && self.selectedItems.splice(0);
                    }
                });
            }
        });
    ...
    Controller.$inject = ['$attrs', '$element', '$q', '$scope', '$rootScope'];
    ...
    function postLink(scope, element, attrs, tableCtrl) {
        ...
        if (angular.isFunction(scope.trigger)) {
            controller.pullTrigger = function () {
                ...
                $q.when(scope.trigger.call(controller.content, controller.order))['finally'](deferred.resolve);
            };
        }
    }
...
function mdDataTablePagination($rootScope, $q) {
    'use strict';

    function postLink(scope, element, attrs) {
        var findTable, setTrigger;
        ...
        if (angular.isFunction(scope.trigger)) {
            ...
            findTable = function (element, callback) {
...
//                scope.$watch(tableCtrl.deferred, function () {
//                    scope.deferred = tableCtrl.deferred;
//                });

                scope.pullTrigger = function () {
//                    var deferred = tableCtrl.defer();
                    scope.trigger.call(scope.content, scope.page, scope.limit);
//                    $q.when(scope.trigger.call(scope.content, scope.page, scope.limit)).finally(function () {
//
//                    });
                };
            };
    ...
    function Controller($scope, $timeout) {
        ...
        $scope.hasNext = function () {
            return !$scope.deferred && (($scope.page * $scope.limit) < $scope.total);
        };

        $scope.hasPrevious = function () {
            return !$scope.deferred && ($scope.page > 1);
        };
...
mdDataTablePagination.$inject = ['$rootScope', '$q'];
    ...
    function postLink(scope, element, attrs, tableCtrl) {
        ...
        if (angular.isDefined(attrs.mdSelectRow)) {
            ...
            scope.toggleRow = function (item, event) {
                ...
                if (tableCtrl.singleSelected) {
                    tableCtrl.selectedItems.length = 0;
    ...
    function postLink(scope, element, attrs, tableCtrl) {
        ...
        tableCtrl.isReady.body.promise.then(function () {
            ...
            scope.toggleAll = function () {
                ...
                if(tableCtrl.singleSelected) return;
...
angular.module('templates.md-data-table-pagination.html', []).run(['$templateCache', function ($templateCache) {
    ...
    $templateCache.put('templates.md-data-table-pagination.html',
        ...
        '  <md-select ng-model="limit" ng-disabled="deferred" md-container-class="md-pagination-select" ng-change="onSelect()" aria-label="Row Count" placeholder="{{rowSelect ? rowSelect[0] : 5}}">\n' +
